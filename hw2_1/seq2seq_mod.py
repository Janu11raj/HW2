...
def avi(files_dir):
    ...
    files = os.listdir(training_feats)
    for file in files:
        file_path = os.path.join(training_feats, file)
        if os.path.exists(file_path):
            print(f"Loading file: {file_path}")
            try:
                value = np.load(file_path)
                print(f"File loaded with shape: {value.shape}")
                
                if value.shape == (80, 4096):
                    avi_data[file.split('.npy')[0]] = value
                else:
                    print(f"File {file_path} has an unexpected shape {value.shape}. Skipping this file.")
...
def main():
    ...
    train_dataloader = DataLoader(dataset = train_dataset, batch_size=32, shuffle=True, num_workers=1, collate_fn=minibatch)
